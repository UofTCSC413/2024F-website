...
<center>
![](imgs2/polynomial_1.png){ height="380" }
![](imgs2/polynomial_3.png){ height="380" }
</center>

::: {.notes}
Example of poor generalization:
- green: data generating process / red: fit.
- Plot 1: datapoints fitted poorly.
- Plot 2: datapoints fitted overly well.
...
<center>
![](imgs2/bottleneck.png){ height="275" }
</center>
...
<center>
![](imgs2/polynomial_overfit.png){ height="450" }
</center>
...
We choose the checkpoint with the best validation error to
actually use. (And if there is a tie, use the **earlier** checkpoint)
...
If you're using sigmoid units, and the weights start out small,
then the inputs to the activation functions take only a small
range of values.

. . .

- The neural network starts out approximately linear,
  and gradually becomes non-linear (and thus more powerful)

## Ensembles

If a loss function is convex (with respect to the predictions),
you have a bunch of predictions for an input, and you don't
know which one is best, you are always better off averaging them!
...
This suggests regularizing them by injecting noise into the computations, a
strategy known as **stochastic regularization**.
...
<center>
![](imgs2/neural_net2.jpeg){ height=30% }
</center>
...
- Can be seen as training an ensemble of 2D different
architectures with shared weights (where D is the number of units)
...
- Since the weights are on $1-p$ fraction of the time,
multiplying the weights by $1-p$ matches the expected value
of the activation magnitude (e.g. going into the next layer).
...
- **Intuitive definition** An algorithm is _differentially private_ if the addition or removal of a single data point does not significantly affect the output.

. . .

- **Formal definition:** An algorithm is _$\varepsilon$-differentially private_ if for all datasets \( D \) and \( D' \) differing in one element, and for all outputs \( S \):
...
## DP in Deep Learning
**Applying DP in Neural Networks:** 
Introduce noise to gradients during training.
...
<center>
![](imgs2/fluctuations.png){ height="250" }
</center>
...
- An overview of gradient descent algorithms: [https://ruder.io/optimizing-gradient-descent](https://ruder.io/optimizing-gradient-descent)

- CS231n: [https://cs231n.github.io/neural-networks-3/](https://cs231n.github.io/neural-networks-3/)
...
- No! It is possible for a model to fit well to the training set, but fail to *generalize*
- We want to know how well the model performs on *new data* that we didn't already
  use to optimize the model
...
- If we use the test set to make modeling decisions, then we will overestimate
...
Dropout can be seen as training an ensemble of 2D different
architectures with shared weights (where D is the number of units)
