...
The machine learning approach requires the minimization of
some cost/loss function, which is often done using
some variation of **gradient descent**.
...
- Autograd, JAX, PyTorch, TensorFlow are examples of particular implementations
  of autodiff, i.e. different libraries
...
You will probably never have to implement autodiff yourself but it is
good to know its inner workings!


## Autodiff, more generally

**Key Insight**: For any new deep learning model that we can come up with,
if each step of our computation is differentiable, then we can train
that model using gradient descent.
...
We wish to compute the gradients $\frac{\partial y_k}{\partial x_i}$ for each $k$ and $i$,
at some $\bf{x}$.
...
Suppose $f = f_2 \circ f_1$, so we have the computations ${\bf y} = f_2 \circ f_1(\bf{x})$, or in other words:
\begin{align*}
{\bf z} &= f_1(\bf{x}) \\
{\bf y} &= f_2(\bf{z})
\end{align*}

If $f_1$ and $f_2$ are primitive operations with simple Jacobians, we can apply the
**Jacobian chain rule**:
...
[https://www.youtube.com/watch?v=wG_nF1awSSY](https://www.youtube.com/watch?v=wG_nF1awSSY)
...
Suppose ${\bf y}$ is a scalar, and represents the loss $\mathcal{L}$ that we wish to minimize.
\begin{align*}
{\bf z} &= f_1(\bf{x}) \\
...
**Summary:** For each primitive operation, we don't need to be able to compute
entire Jacobian matrix. **We need to be able to compute the vector-Jacobian product.**
...
The VJP function should takes in the output gradient (i.e. ${\bar y}$),
the answer ($y$), and the arguments ($x$), and returns the input gradient (${\bar x}$). Here are some examples from [https://github.com/mattjj/autodidact/blob/master/autograd/numpy/numpy_vjps.py](https://github.com/mattjj/autodidact/blob/master/autograd/numpy/numpy_vjps.py)
...
This design provides **modularity!** Each node needs to know how
to compute its outgoing messages, i.e. the VJPs corresponding to
each of its parents (arguments to the function).
...
[https://arxiv.org/pdf/1606.04474.pdf](https://arxiv.org/pdf/1606.04474.pdf)
...
In maximum likelihood training, we want to maximize
$$\prod_{i=1}^N p\left({\bf s}^{(i)}\right)$$

<!-- $_$ -->

Or minimize:
$$-\sum_{i=1}^N \log p\left({\bf s}^{(i)}\right)$$
...
\end{align*}
We can make a simplifying **Markov assumption** that the distribution over
the next word depends on the preceding few words.

## Probability of a sentence

In assignment 1, we use a context length of 3 and model:
\begin{align*}
...
A simple way of modeling $p(w_t | w_{t-2}, w_{t-1})$ is by constructing
...
$$p(w_3 = {\rm cat} | w_1 = {\rm the}, w_2 = {\rm fat}) = \frac{{\rm count}({\rm the\ fat\ cat})}{{\rm count}({\rm the\ fat})}$$
...
<center>
![From [https://lagunita.stanford.edu/c4x/Engineering/CS-224N/asset/slp4.pdf](https://lagunita.stanford.edu/c4x/Engineering/CS-224N/asset/slp4.pdf)](imgs/shakespeare.png)
</center>
...
<center>
![](imgs/distributed_cartoon.png){ height="500" }
</center>
...
<center>
![](imgs/model2.png){ height="450" }
</center>

## Word Representations

Since we are using one-hot encodings for the words,
the weight matrix of the word embedding layer acts like a lookup table.

<center>
![](imgs/lookup_as_linear.png){ height="350" }
</center>
...
- Global Vector (GloVe) embeddings are a simpler and faster approach based on a matrix factorization
  similar to principal component analysis (PCA)

Idea: First fit the distributed word representations using GloVe,
then plug these embeddings into a neural net that does some other task
(e.g. translation)
...
Consider a **co-occurrence matrix** $X$, which counts
the number of times the words appear nearby (say, less than 5 positions apart)
...
> The cat got squashed in the garden on Friday.
> The dog got flattened in the yard on Thursday.
...
**Key insight**: The co-occurrence matrix of words contain information about the
semantic information (meaning) of words
...
<!-- - Hence, more similar words are more likely to co-occur (Lisa: I think this interpretation is wrong)-->
...
[https://colab.research.google.com/drive/1aNbE6HcawVF67RV0hWi4qK33Um7cKykr?usp=sharing](https://colab.research.google.com/drive/1aNbE6HcawVF67RV0hWi4qK33Um7cKykr?usp=sharing)


## Key idea from the Demo

- Distances are somewhat meaningful, and are based on **word co-occurrences**
    - the words "black" and "white" will have similar embeddings because
      they co-occur with similar other words.
    - "cat" and "dog" is more similar to each other than "cat" and "kitten"
      because the latter two words occur in *different contexts*!
...
Word embeddings are inherently biased because there is bias in the
training data.

Neural networks learn patterns in the training data, so if the training
data contains human biases, then so will the trained model!
