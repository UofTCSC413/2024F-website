...
**Transfer Learning** is the idea of using weights/features trained
on one task, and using it on another task.
...
<center>
![](imgs/alexnet.png){height="200"}
</center>

## Transfer Learning with CNN

<center>
![](imgs/alexnet.png){height="250"}
</center>
...
<center>
![](imgs/alexnet.png){height="170" }
![](imgs/vgg.png){height="170"}
</center>
...
- See [https://cs231n.github.io/convolutional-networks/](https://cs231n.github.io/convolutional-networks/).
...
This is more frequently done on pixel-wise prediction problems, which
...
Interpretation of deep learning models is a challenge, but there are some tricks we can use
to interpret CNN models
...
<center>
![](imgs/mlp_weights.png){ height="300" }
![](imgs/zeiler_layer1_filters.png){ height="300" }
</center>
...
<center>
![](imgs/comp.png){height="300"}
</center>

From this graph, we could compute $\frac{\partial L}{\partial x}$ -- the model's sensitivity
...
From: [https://proceedings.neurips.cc/paper/2018/file/294a8ed24b1ad22ec2e7efea049b8737-Paper.pdf](https://proceedings.neurips.cc/paper/2018/file/294a8ed24b1ad22ec2e7efea049b8737-Paper.pdf)
...
Requires a few tricks to make this work; see [https://distill.pub/2017/feature-visualization/](https://distill.pub/2017/feature-visualization/)
...
<center>
![](imgs/adversarial.png){height="300"}
</center>

. . .

**Producing adversarial images**:
Given an image for one category (e.g. panda), compute the image
gradient to maximize the network’s output unit for a different
category (e.g. gibbon)
...
Adversarial examples transfer to different networks trained on a totally
separate training set!

**White-box Adversarial Attack**: Model architecture and weights are known,
so we can compute gradients. (What we've been doing so far in the demos)

## White-box vs Black-box Adversarial Attacks

**Black-box Adversarial Attack**: Model architecture and weights
are unknown.

- You don’t need access to the original network!
- You can train up a new network to match its predictions, and then
	construct adversarial examples for that.

Attack carried out against proprietary classification networks accessed
using prediction APIs (MetaMind, Amazon, Google)

## Adversarial Examples in 3D

It is possible to have a 3D object that gets misclassified by a
neural network from all angles.

[https://www.youtube.com/watch?v=piYnd_wYlT8](https://www.youtube.com/watch?v=piYnd_wYlT8)

## Printed Adversarial Examples

It is possible for a printed image to cause object detection
to fail.
...
- How sensitive is $L(u,v; \hat{\theta}(\epsilon))$ to a perturbation $\hat{\theta}(\epsilon)$ where we have
upweighted the training point $(x,y)$?
