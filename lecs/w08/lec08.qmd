...
<center>
![](imgs/autoregressive.png){height="270"}
</center>
...
<center>
![](imgs/rnn_motivation.png){height=30%}
</center>
...
This simple RNN takes a sequence of numbers as input (scalars),
and sums its inputs.

<center>
![](imgs/rnn_example1_sol.png){height="500"}
</center>

## Simple RNN Example 2: Comparison

This RNN takes a sequence of **pairs of numbers** as input,
and determines if the total values of the first or second input are larger:

<center>
![](imgs/rnn_example2_sol.png){height="450"}
</center>
...
Let's find weights and biases for the RNN on the right so that it computes the parity.
All hidden and output units are **binary threshold units** ($h(x) = 1$ if $x > 0$ and $h(x) = 0$ otherise).
...
<center>
![](imgs/parity_rnn_template_new.png){height="500"}
</center>

## Unrolling Parity RNN

<center>
![](imgs/parity_rnn_unroll.png){height="500"}
</center>
...
<center>
![](imgs/parity_rnn_solution.png){height="300"}
</center>
...
* $h_1$ computes _and_ by subtracting -1.5 from the sum of $y$ and the current input $x$ and then applying the binary threshold activation function.
* $h_2$ computes _or_ by subtracting -0.5 from the sum of $y$ and the current input $x$ and then applying the binary threshold activation function.
...
<center>
![](imgs/bptt_rnn_unroll.png){height="300"}
</center>
...
<center>
![](imgs/unrolled_computation_graph.png){height="470"}
</center>

## What can RNNs compute?

In 2014, Google researchers built an encoder-decoder RNN that learns to
execute simple Python programs, one character at a time! [https://arxiv.org/abs/1410.4615](https://arxiv.org/abs/1410.4615)
...
<center>
![](imgs/rnn4.png){height="500"}
</center>

## Sentiment140 Data

Dataset of tweets with either a positive or negative emoticon, but with
the emoticon removed.
...
The longer your sequence, the longer gap the time step
between when we see potentially important information
and when we need it:

<center>
![](imgs/grad.png){height="300"}
</center>
...
<center>
![](imgs/encoder_uni.png){height="200"}
</center>
...
Another way to look at why gradients explode or vanish is that
we are applying a function over and over again.

. . .

Each hidden layer computes some function of previous hidden layer
and the current input: ${\bf h}^{(t)} = f({\bf h}^{(t-1)}, {\bf x}^{(t)})$
...
Note that the function values gravitate towards **fixed points**,
and that the derivatives becomes either **very large** or **very small**.
...
<center>
![](imgs/tanh_responses.png){height="400"}
</center>
...
<center>
![](imgs/cliffs.png){ height=40% }
</center>

## Cliffs II

Generally, the gradient will explode on some inputs and vanish on others.
In expectation, the cost may be fairly smooth.

## Gradient Clipping

One solution is to "clip" the gradient so that it has a norm of at most $\eta$.
Otherwise, update the gradient ${\bf g}$ with ${\bf g} \leftarrow \eta\frac{{\bf g}}{||{\bf g}||}$
...
Hidden units are a kind of memory. Their default behaviour
should be to **keep their previous value**.

. . .

If the function ${\bf h}^{(t)} = f({\bf h}^{(t-1)}, {\bf x}^{(t)})$
is close to the identity, then the gradient computations $\displaystyle \frac{\partial {\bf h}^{(t)}}{\partial {\bf h}^{(t-1)}}$
are stable.

. . .

This initialization allows learning much longer-term dependencies
than "vanilla" RNNs

## Long-Term Short Term Memory

Change the **architecture** of the recurrent neural network
by replacing each single unit in an RNN by a "memory block":

<center>
![](imgs/lstm.png){height=50%}
</center>

## LSTM

<center>
![](imgs/lstm_jimmy.png){height="550"}
</center>
...
During training, we try to get the RNN to generate one
particular sequence in the training set. At each time step:
...
<center>
![](imgs/rnn_gen_figure.png){height="150"}
</center>
...
<center>
![](imgs/rnn_gen_figure.png){height="400"}
</center>
...
- In some languages, it's hard to define what should be
considered a word.
